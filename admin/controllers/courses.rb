GradebookApp::Admin.controllers :courses do
  get :index do
    @title = "Courses"
    @courses = Course.all
  ...
  delete :destroy_many do
    ...
    ids = params[:course_ids].split(',').map(&:strip)
    courses = Course.find(ids)
    
    if Course.destroy courses
    
      flash[:success] = pat(:destroy_many_success, :model => 'Courses', :ids => "#{ids.join(', ')}")
    end
    redirect url(:courses, :index)
  end
end
